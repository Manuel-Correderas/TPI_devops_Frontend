...
from auth_helpers import get_backend_url, auth_headers  # 👈 usamos helpers comunes
...
.name {
  ...
  margin:8px 0 4px; text-align:center; font-size:1rem;
}
.price {
...
.meta {
  color:#444; text-align:center; font-size:.9rem; margin:0 0 8px;
}
...
def require_login_for_cart() -> bool:
    """
    Mensaje amigable si no hay sesión para usar el carrito.
    (No corta toda la página, solo muestra aviso y link a login).
    """
    if "auth_token" not in st.session_state:
        st.warning("Tenés que iniciar sesión para agregar productos al carrito.")
        # ⚠️ Asegurate que el archivo exista con este nombre
